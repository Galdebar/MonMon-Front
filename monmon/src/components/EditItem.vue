	...
	<div id="editItemView" v-bind:class="{ hidden: !isShown }">
		<div class="container popup-container small-padding">
			<div class="top no-wrap-flex">
				...
				<h1>{{ headerTitle }}</h1>
				<BtnStandard v-on:action="triggerView">
					X
				</BtnStandard>
			</div>
			<div class="middle">
				<div id="name" class="section">
					<input type="text" v-model="shoppingItem.itemName" />
				</div>

				<div id="quantity" class="section">
					<h4>Quantity</h4>
					<div class="input">
						<BtnStandard v-on:action="decreaseQuantity">
							-
						</BtnStandard>
						<input type="number" v-model="shoppingItem.quantity" />
						<BtnStandard v-on:action="increaseQuantity">
							+
						</BtnStandard>
					</div>
				</div>
			...
			<div class="bottom no-wrap-flex">
				...
				<BtnStandard v-on:action="updateItem">
					Save
				</BtnStandard>
			</div>
		</div>
...
export default {
	...
	methods: {
		async updateItem() {
			let isResponseOK = await this.$store.dispatch(
				"updateItem",
				this.shoppingItem
			);
			if (isResponseOK) {
		...
		async deleteItem() {
			let responseIsOk = await this.$store.dispatch(
				"deleteItem",
				this.shoppingItem
			);
			if (responseIsOk) {
